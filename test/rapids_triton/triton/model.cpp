...
bool
TestBoolString(std::string b)
{
  auto model_config = std::make_unique<common::TritonJson::Value>(
      common::TritonJson::ValueType::OBJECT);
  model_config->AddInt("max_batch_size", 1);
  model_config->Add(
      "output",
      common::TritonJson::Value(common::TritonJson::ValueType::ARRAY));
  auto params =
      common::TritonJson::Value(common::TritonJson::ValueType::OBJECT);
  auto string_value =
      common::TritonJson::Value(common::TritonJson::ValueType::OBJECT);
  ...
  EXPECT_THROW(
      {
        try {
          ...
          EXPECT_STREQ(
              "Expected 'true' or 'false' for parameter 'some_bool', got: "
              "'True'",
              e.what());
          throw;
        }
