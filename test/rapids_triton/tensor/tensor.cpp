...
TEST(RapidsTriton, multi_buffer_tensor)
{
  ...
  if constexpr (IS_GPU_BUILD) {
    mem_type = DeviceMemory;
  }
  std::transform(
      data.begin(), data.end(), std::back_inserter(all_buffers),
      [mem_type](auto& elem) {
        return Buffer<int>{&elem, 1, mem_type};
      });
  auto tensor = Tensor<int>(
      shape, all_buffers.begin(), all_buffers.end(), mem_type, 0,
      cudaStream_t{});

  auto data_out = std::vector<int>(data.size());
#ifdef TRITON_ENABLE_GPU
  cudaMemcpy(
      static_cast<void*>(data_out.data()), static_cast<void*>(tensor.data()),
      sizeof(int) * tensor.size(), cudaMemcpyDeviceToHost);
#else
  std::memcpy(
      static_cast<void*>(data_out.data()), static_cast<void*>(tensor.data()),
...
TEST(RapidsTriton, tensor_multi_copy)
{
  ...
  std::transform(
      data.begin(), data.end(), std::back_inserter(receivers),
      [&receiver_shape](auto& val) {
        return Tensor<int>(
            receiver_shape, Buffer<int>{std::size_t{1}, HostMemory});
      });
  ...
  std::transform(
      receivers.begin(), receivers.end(), std::back_inserter(data_out),
...
TEST(RapidsTriton, retained_output_buffer_outlives_output_tensor)
{
  ...
  auto retained_buffer =
      std::make_shared<Buffer<int>>(expected.size(), HostMemory);
  auto* source_data = retained_buffer->data();

  {
    auto view = Buffer<int>(
        source_data, retained_buffer->size(), retained_buffer->mem_type(),
        retained_buffer->device(), retained_buffer->stream());
    auto output = OutputTensor<int>(
        std::vector<std::size_t>{expected.size()}, std::move(view), "output",
        responder);
  ...
  EXPECT_THAT(
      std::vector<int>(source_data, source_data + expected.size()),
