...
struct SharedModelState {
  ...
  explicit SharedModelState(
      std::unique_ptr<common::TritonJson::Value>&& config,
      ...
      : config_{std::move(config)},
        ...
        output_shapes_([this, squeeze_output]() {
          ...
          // Using a raw loop because TritonJSON::Value access has no iterator
          // interface
          for (std::size_t i = 0; i < output_entries.ArraySize(); ++i) {
            ...
            // The squeeze_output option was introduced to handle a bad choice
            // of convention in the original FIL backend implementation. For
            // legacy compatibility, we introduced this option into
            // RAPIDS-Triton, but in general, new backends are advised to avoid
            // using it and defer this sort of flattening operation to the
            // consumer.
            if (squeeze_output) {
              shape.erase(
            ...
            result.insert(
                std::upper_bound(
                    std::begin(output_shapes_), std::end(output_shapes_), name,
  ...
  auto get_output_names() const
  {
    ...
    std::transform(
        std::begin(output_shapes_), std::end(output_shapes_),
        std::back_inserter(output_names),
        [](auto& output_shape) { return output_shape.first; });
    return output_names;
  }
 ...
 private:
  ...
  std::vector<
      std::pair<std::string, std::vector<std::int64_t>>> mutable output_shapes_;

  template <typename T>
  auto get_config_param(
      std::string const& name, std::optional<T> const& default_value)
  {
    auto result = T{};
    ...
    if (config_->Find("parameters", &parameters) &&
      ...
      if constexpr (std::is_same_v<T, bool>) {
        if (string_repr == "true") {
          ...
          throw TritonException(
              Error::Internal, "Expected 'true' or 'false' for parameter '" +
                                   name + "', got: '" + string_repr + "'");
        }
      } else {
        input_stream >> result;
      }
      ...
      if (default_value) {
        ...
        throw TritonException(
            Error::InvalidArg, std::string("Required parameter ") + name +
                                   std::string(" not found in config"));
      }
    }
