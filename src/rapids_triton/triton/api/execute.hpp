    ...
    std::size_t request_count)
{
  ...
  try {
    auto* model_state =
        get_model_state<ModelState>(*get_model_from_instance(*instance));
    auto* instance_state = get_instance_state<ModelInstanceState>(*instance);
    auto& model = instance_state->get_model();
    auto max_batch_size =
        model.template get_config_param<std::size_t>("max_batch_size");
    ...
    auto output_shape_fetcher =
        [&model](std::string const& name, Batch::size_type batch_dim) {
          ...
          std::transform(
              std::begin(config_shape), std::end(config_shape),
              std::back_inserter(result), [](auto& coord) {
                if (coord < 0) {
                  throw TritonException(
                      Error::Internal,
                      "Backends with variable-shape outputs must request "
    ...
    auto statistics_reporter =
        [instance](
            TRITONBACKEND_Request* request, time_point req_start,
            time_point req_comp_start, time_point req_comp_end,
            time_point req_end) {
          report_statistics(
              *instance, *request, req_start, req_comp_start, req_comp_end,
              req_end);
        };

    auto batch = Batch(
        raw_requests, request_count, *(model_state->TritonMemoryManager()),
        std::move(output_shape_fetcher), std::move(statistics_reporter),
    ...
    try {
      ...
      predict_err = TRITONSERVER_ErrorNew(
          Error::Internal, "prediction failed with unknown exception");
    }
    ...
    report_statistics(
        *instance, request_count, start_time, compute_start_time,
    ...
    result = TRITONSERVER_ErrorNew(
        Error::Internal, "execution failed with unknown exception");
  }
