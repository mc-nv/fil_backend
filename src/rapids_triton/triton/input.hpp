...
inline auto*
get_triton_input(TRITONBACKEND_Request* request, std::string const& name)
{
...
auto
get_triton_input_shape(
    Iter requests_begin, Iter requests_end, std::string const& name)
{
  ...
  auto batch_dim = std::accumulate(
      requests_begin, requests_end, int64_t{},
      [&reported_dtype, &input_shape, &input_dims, &name](
          auto total, auto& request) {
        auto* input = get_triton_input(request, name);
        triton_check(TRITONBACKEND_InputProperties(
            input, nullptr, &reported_dtype, &input_shape, &input_dims, nullptr,
            nullptr));

        if (reported_dtype != TritonDtype<T>::value) {
          auto log_stream = std::stringstream{};
          log_stream << "incorrect type " << reported_dtype
  ...
  std::transform(
      input_shape, input_shape + input_dims, std::back_inserter(result),
