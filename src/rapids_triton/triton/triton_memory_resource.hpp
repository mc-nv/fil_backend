...
struct triton_memory_resource {
  triton_memory_resource(
      TRITONBACKEND_MemoryManager* manager, device_id_t device_id,
 ...
 public:
  void* allocate(
      cuda::stream_ref stream, std::size_t bytes,
      std::size_t alignment = rmm::CUDA_ALLOCATION_ALIGNMENT)
  {
    ...
    if (manager_ == nullptr) {
      ...
      triton_check(TRITONBACKEND_MemoryManagerAllocate(
          manager_, &ptr, TRITONSERVER_MEMORY_GPU, device_id_,
  ...
  void deallocate(
      cuda::stream_ref stream, void* ptr, std::size_t bytes,
      std::size_t alignment = rmm::CUDA_ALLOCATION_ALIGNMENT)
  {
    if (manager_ == nullptr) {
  ...
  constexpr friend void get_property(
      const triton_memory_resource&, cuda::mr::device_accessible) noexcept
  {
  }
};
...
static_assert(
    cuda::mr::resource_with<
        triton_memory_resource, cuda::mr::device_accessible>,
