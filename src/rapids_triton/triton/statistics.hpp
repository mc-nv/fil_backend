...
report_statistics(
    ...
    time_point start_time, time_point compute_start_time,
    time_point compute_end_time, time_point end_time)
{
  triton_check(TRITONBACKEND_ModelInstanceReportStatistics(
      &instance, &request, true, start_time.time_since_epoch().count(),
...
inline void
report_statistics(
    TRITONBACKEND_ModelInstance& instance, std::size_t request_count,
    time_point start_time, time_point compute_start_time,
    time_point compute_end_time, time_point end_time)
{
  triton_check(TRITONBACKEND_ModelInstanceReportBatchStatistics(
      &instance, request_count, start_time.time_since_epoch().count(),
      compute_start_time.time_since_epoch().count(),
      compute_end_time.time_since_epoch().count(),
      end_time.time_since_epoch().count()));
}
